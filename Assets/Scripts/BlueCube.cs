	...
	void OnCollisionEnter (Collision col) {
		...
		if (upCube != true && _cubeLocation.isBusy != true) {
			...
			case 0: // GroundToTop
				PlaySoundFX (0);
				StartCoroutine(Animate(groundToTop));
				col.rigidbody.velocity += groundToTop * jumpForce;
				break;

			case 1: // TopToGround
				PlaySoundFX (0);
				StartCoroutine(Animate(topToGround));
				col.rigidbody.velocity += topToGround * jumpForce;
				break;

			case 2: // FrontToBack
				PlaySoundFX (0);
				StartCoroutine(Animate(frontToBack));
				col.rigidbody.velocity += frontToBack * jumpForce;
				break;

			case 3: // BackToFront
				PlaySoundFX (0);
				StartCoroutine(Animate(backToFront));
				col.rigidbody.velocity += backToFront * jumpForce;
				break;

			case 4: // LeftToRight
				PlaySoundFX (0);
				StartCoroutine(Animate(leftToRight));
				col.rigidbody.velocity += leftToRight * jumpForce;
				break;

			case 5: // RightToLeft
				PlaySoundFX (0);
				StartCoroutine(Animate(rightToLeft));
				col.rigidbody.velocity += rightToLeft * jumpForce;
				break;
			}
