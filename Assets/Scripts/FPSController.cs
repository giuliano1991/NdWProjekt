...
public class FPSController : MonoBehaviour {
	...
	public float distanceToCube = 10f;
	...
	void Update () {
		...
		if (Input.GetMouseButtonDown (0)) {
			...
			if (Physics.Raycast (ray, out hit, distanceToCube)) {
				...
				else if (hit.collider.GetComponent<PurpleButton> () != null) {

						PurpleButton pb = hit.collider.GetComponent<PurpleButton> ();
						pb.LeftTrigger ();
				...
				// Wir geprüft ob der Cube ein Elevatorknopf ist
				else if (hit.collider.GetComponent<Elevator> () != null) {

					Elevator eb = hit.collider.GetComponent<Elevator> ();
					eb.ButtonPress ();
