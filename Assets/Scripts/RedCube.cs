...
public class RedCube : MonoBehaviour {
    ...
    // Öffentliche Floatvariablen für das maximale Ziehen und Drücken des Cubes + die Kraft die ausgeübt auf der Kugel
	...
	private bool gotPull = false;

    // Startposition, Endposition, Distanz und Geschwindigkeit
	...
	public RedCubeTrigger _RedCubeTrigger;
...
//-------------------------------------------------------
   ...
   /* void OnCollisionEnter(Collision col)
	...
	////////////////////////////
	// Die Kollisionsabfrage mit der Kugel
	public void TriggerFunction (GameObject col)
	{
			switch (_cubeLocation.location)
			{
			case 0:
				col.GetComponent<Rigidbody>().velocity = groundToTop * force;
				break;

			case 1:
				col.GetComponent<Rigidbody>().velocity = topToGround * force;
				break;

			case 2:
				col.GetComponent<Rigidbody>().velocity = frontToBack * force;
				break;

			case 3:
				col.GetComponent<Rigidbody>().velocity = backToFront * force;
				break;

			case 4:
				col.GetComponent<Rigidbody>().velocity = leftToRight * force;
				break;

			case 5:
				col.GetComponent<Rigidbody>().velocity = rightToLeft * force;
				break;
			}
	}
...
//-------------------------------------------------------
    ...
    IEnumerator Animate(Vector3 fromTo)
    {
		...
		if (haveCollision != false && gotPull == true) {
		
			_RedCubeTrigger.ActiveTrigger();
		...
		if (haveCollision != false) {
			...
			_RedCubeTrigger.DeactiveTrigger();
